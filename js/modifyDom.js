...
function constructExpenseTableBody(friendId) {
	...
	for (i in allInvolvedExpenseIds) {
		...
		var detailRow = $('<tr style="display:none"></tr>');
		...
		detailRow.append($('<td>Total before split: $' + expense.total + '</td>'));
		detailRow.append($('<td>Date of expense: ' + expense.date_added + '</td>'));
